...
const getContextImport = (useContext) => useContext ? 'import { useTheme } from "./contextApi/context";\n' : "";
const getReduxImport = (useRedux) => {
    if (!useRedux)
        return "";
    return `import { decrement, increment, reset, selectCounterValue } from "./redux/slices/counterSlice";\nimport { useAppDispatch, useAppSelector } from "./redux/hooks";\n`;
};
const getShadcnImport = (useShadcn) => useShadcn
    ? 'import { Button } from "@/components/ui/button";\nimport { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";\n'
    : "";
const getApiClientImport = (language, apiClient) => {
    if (apiClient === "None")
        return "";
...
const getThemeHelpers = (useContext) => useContext
    ? `
...
`
    : "";
const getReduxHelpers = (useRedux, useShadcn) => useRedux
    ? `
...
`
    : "";
const getApiHelpers = (apiClient) => apiClient !== "None"
    ? `
...
`
    : "";
const getShellClassName = (useContext) => useContext
    ? '{`min-h-screen transition-colors duration-300 ${theme === "dark" ? "bg-slate-950 text-slate-100" : "bg-stone-50 text-slate-900"}`}'
    : '"min-h-screen bg-slate-950 text-slate-100"';
const getHeroCards = (useContext, useRedux, appExtension, starter) => `        <div className="grid gap-4 sm:grid-cols-3">
...
const getAppBody = ({ projectName, language, useRedux, useShadcn, useContext, useRouter, authMode, apiClient, starter, }) => {
...
const getAppTemplate = (options) => `import "./index.css";
